...
- **Core moves land here too**, per the measured candidate list in `tsdoctor-package-architecture.md`. One correction to this roadmap's own wording: `llms-processing.ts` is pure text transforms over a cross-framework standard and belongs in core — only `llms-program.ts` (I/O, RSPress `outDir`) is what "llms.txt wiring stays in the adapter" describes.
