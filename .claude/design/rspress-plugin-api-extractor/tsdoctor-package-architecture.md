...
> **Target architecture, now realized.** This describes the package architecture for the `@tsdoctor` org. Phases 1, 2, 4 and the phase 5 alpha have landed, plus the Tier 1 core moves: all seven core packages (`@tsdoctor/vfs`, `@tsdoctor/registry`, `@tsdoctor/model`, `@tsdoctor/bundle`, `@tsdoctor/snapshot`, `@tsdoctor/seo`, `@tsdoctor/pages`) exist as workspaces under `packages/`, and two adapters live under `platforms/` — `rspress/` and, since phase 5, `vitepress/`, the second consumer that proves the seams. See `build-architecture.md` for the adapter's current shape. Phasing is governed by `roadmap-1.0.md`; the phase 1 executed record is in `monorepo-consolidation.md`.
...
As of 2026-09-02 phases 1, 2 and 4 have been executed (phase 1 released via PR #163; phase 2 landed on `feat/tsdoctor-phase-2`, all gates green — suite 1,314/1,315, typecheck 23/23 — releases pending via changesets; phase 4 landed on `feat/phase-4`), the Tier 1 core moves landed on `feat/tsdoctor-vfs`, and the phase 5 alpha landed on `feat/phase-5` (2026-09-03, close-out in progress). All seven packages exist as in-repo workspaces:
...
- `packages/pages` — `@tsdoctor/pages` (phase 5): the framework-neutral page IR — the block vocabulary, `prepareWorkItems` (work items plus the route map, uncategorized items and collisions returned as data), the `ApiItem` → `Page` builders, the navigation tree, example preparation, the plain-markdown emitter and the llms.txt text transforms. See `doc-ir-and-pages.md`.
- `platforms/rspress/` — the RSPress adapter: hooks, React runtime, remark plugins, the MDX and `_meta.json` emitters over the IR, llms.txt wiring, and the thin adapter seams over the seven core packages.
- `platforms/vitepress/` — `vitepress-plugin-api-extractor` (phase 5 alpha): the awaited `apiExtractor()` config helper, the markdown / frontmatter / sidebar emitters over the IR, and the native `@shikijs/vitepress-twoslash` wiring. Markdown-only. See `platforms/vitepress/CLAUDE.md`.
...
| `@tsdoctor/pages` | framework-neutral doc IR: typed blocks + mdast prose, `prepareWorkItems`, the `ApiItem` → `Page` builders, the nav tree, the plain-markdown emitter, llms.txt text transforms | **exists** (`packages/pages`, phase 5); the RSPress page generators lifted as a characterization behind a golden gate |
...
| `rspress-plugin-api-extractor` | thin adapter: RSPress hooks, React runtime, remark plugins, the MDX emitter, llms.txt wiring | `platforms/rspress/`, consuming all seven core packages |
| `vitepress-plugin-api-extractor` | thin adapter: the awaited config helper, markdown / sidebar / frontmatter emitters, native Twoslash wiring | **exists** (`platforms/vitepress/`, phase 5 alpha) |

`@tsdoctor/pages` was deliberately last: per the settled decision in `roadmap-1.0.md`, the IR was extracted in phase 5 alongside the VitePress adapter so two live consumers shaped it — and the second consumer immediately surfaced two further core moves (`prepareWorkItems`, the Twoslash cache) plus a next tier of duplication, recorded under [Core-Move Candidates](#core-move-candidates).
...
- **`@tsdoctor/pages`** (phase 5, delivered) — `@effected/markdown` as the IR substrate. Division of labor by design: HTML generation belongs to the consuming framework (RSPress/VitePress) — `@effected/markdown`'s role is the TRANSITION step, emitting the correct mdast/MDX-shaped tree (and its serialization) for the framework to consume and render via the `Mdast` projection. MDX (JSX-element nodes) shipped in the kit's 0.7.0 release (construction + serialization only, no MDX parsing) as the round-1 dogfood expansion driven by the RSPress adapter's MDX serialization needs — see candidate (a) below and the proof-consumer test in `packages/model/__test__/mdx-vocabulary.test.ts`; the RSPress MDX emitter now serializes through it, and the escaping round it raised is candidate (h). HTML support isn't needed, not forbidden: the kit's current no-HTML posture is owner policy that could be revisited, but the architecture deliberately keeps HTML framework-side.
...
- (h) **DELIVERED and adopted** — minimal inline escaping in `@effected/markdown` 0.8.0 (raised 2026-09-02 by the phase-5 emitters, answered the same day; release pending): `_` escaped only where it can bind emphasis, `&` only when entity-shaped, `>` only at a line start, `#` in headings only as an ATX closing sequence, and the `{#id}` heading suffix pinned raw on a non-MDX tree. Both downstream byte-parity shims are deleted. Per-node control of the presence-keyed MDX `{` escape was **declined** as a kit design invariant, the documented raw hatch being an inline `Html` node; the empty-JSX self-close and blank-line JSX-sibling separation were confirmed as the `mdast-util-mdx-jsx` oracle defaults. Full record in `doc-ir-and-pages.md`.
...
**Deliberately not a candidate: `path-derivation.ts`.** It reads neutral, but it encodes multiVersion/locale layout conventions that are indistinguishable from RSPress's own from inside this repo — the case the two-consumer rule is actually for. **Watch item: `twoslash-cache.ts`** — **now MOVED** to `@tsdoctor/vfs` (`TwoslashCache.ts`, phase 5), once the VitePress adapter wired the same `TwoslashTypesCache` interface and the two adapters were measured warming one XDG store.

**Tier 2 execution status (2026-09-03).** Taken in phase 5: `llms-processing.ts` and the scope helpers → `@tsdoctor/pages` (`Llms.ts`, `Scope.ts`); `prepareWorkItems` → `@tsdoctor/pages` (`WorkItems.ts`, with uncategorized items and route collisions returned as data and the RSPress adapter keeping a reporting wrapper); the Twoslash result cache → `@tsdoctor/vfs`. Stays in the adapter: `deriveOutputPaths`, `category-resolver.ts` and the observability cluster. **The next tier — measured by building the VitePress adapter, NOT yet taken:** `platforms/vitepress/src/Generate.ts` re-spells the neutral half of RSPress's `config-resolution.ts` (import prepending, dependency extraction, tsconfig resolution, manifest decode → `packageContext`); `Categories.ts` duplicates `DEFAULT_CATEGORIES` plus the override merge; `Registry.ts` duplicates the registry-stack composition and the `"tsdoctor"` namespace literal; and RSPress's `hide-cut-transformer.ts` hand-matches `// ---cut---` instead of using `@tsdoctor/pages`'s directive helpers. Destinations are open; the list lives in `doc-ir-and-pages.md` under Tier 2 core moves.
