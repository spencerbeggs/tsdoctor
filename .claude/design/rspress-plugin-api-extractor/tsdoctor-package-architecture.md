...
- (c) `Store.layerSqlite` should pass through the remaining `SqliteClient.layer` options (e.g. `disableWAL`).
- (d) a `checkpointOnClose: true` option registering the `wal_checkpoint(TRUNCATE)` scope finalizer that every durable-SQLite consumer hand-writes.
- (e) an adoption recipe/API for consumers migrating from the `effect/unstable/sql` Migrator ledger whose pre-Store migrations aren't idempotent.
