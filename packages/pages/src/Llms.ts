...
//
// These handle parsing, filtering, and generating LLMs text files for
// per-package API documentation scopes. They are pure string transforms over
// the cross-framework llms.txt standard with no Effect dependencies, so a
// second adapter emits byte-identical per-package files; the I/O program that
// consumes them (RSPress's llms-program.ts) stays adapter-side.
