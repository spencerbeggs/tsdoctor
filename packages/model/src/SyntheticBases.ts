/**
 * Detection of synthetic base declarations — unexported items API Extractor
 * hoisted into the model (via `includeForgottenExports`) because an exported
 * class's extends clause references them (the `Foo_base` pattern from Effect
 * `Schema.Class` / mixin factories). Pure.
 *
 ...
 * @public
 */
export function detect(items: readonly ApiItem[]): SyntheticBaseDetection {
