...
import { SyntheticBases } from "../src/index.js";
...
describe("SyntheticBases.detect", () => {
	it("detects an unexported base variable referenced by a class extends clause", () => {
		...
		const detection = SyntheticBases.detect(members);
	...
	it("exposes a stable anchor for the inline section", () => {
		...
		expect(SyntheticBases.BASE_CLASS_ANCHOR).toBe("base-class");
	});
});
