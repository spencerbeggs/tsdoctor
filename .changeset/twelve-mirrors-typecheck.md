...
The cache lives in the user's XDG cache directory (`~/.cache/tsdoctor/twoslash.sqlite`) — nothing to commit or gitignore. Each entry is keyed on the code, the compiler options, and the whole type environment, so it never serves a stale result; the trade-off is coarse invalidation, so repeat builds over an unchanged API become nearly free (CI re-runs, prose-only edits, theme changes) while a build right after an API item changes gets no benefit. Every failure path (a missing cache directory, an unreadable entry) degrades to a cache miss rather than failing the build. The end-of-build summary now logs a `Twoslash cache: N/N hits (P%), N entries` line.
