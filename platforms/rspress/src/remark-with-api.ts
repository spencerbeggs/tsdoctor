	...
	return async function remarkTransformer(tree: Root, file: { path?: string; cwd?: string }): Promise<void> {
		...
		visit(tree, "code", (node: Code, index: number | undefined, parent: Parent | undefined) => {
			...
			const promise = (async () => {
				...
				// The scope this block belongs to: it selects both the Twoslash
				// environment (so the block is checked under its own package's
				// tsconfig) and the cross-link routes applied afterwards.
				...
				// Per-block Twoslash attribution. Fresh wrapper per block so the
				// accumulator cannot be raced by concurrently rendered siblings; it
				// delegates to the shared transformer, preserving the tsEnv cache.
				let twoslashMs = 0;
				const twoslashTransformer = getTransformer(apiScope);
				const transformers: ShikiTransformer[] = twoslashTransformer
					? [
							createTwoslashTimingWrapper(twoslashTransformer, (duration) => {
								twoslashMs += duration;
							}),
						]
					: [];
				...
				emitEvent(
					PE.CodeBlockProcessed({
						...
						// Twoslash runs entirely inside the transformer's `preprocess` hook,
