...
describe("TypeAliasPageGenerator", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(Tsdoc.summary).mockReturnValue("A type alias for values");
	...
	describe("generate", () => {
		...
		it("should include source link toolbar", async () => {
			vi.mocked(ApiItems.sourceLink).mockReturnValue("https://github.com/user/repo/blob/main/src/types.ts#L15");
		...
		it("should include LLMS plugin section when enabled", async () => {
			vi.mocked(ApiItems.sourceLink).mockReturnValue("https://github.com/user/repo/blob/main/src/types.ts");
		...
		it("should pass sourceConfig to getSourceLink", async () => {
			...
			expect(ApiItems.sourceLink).toHaveBeenCalledWith(mockApiTypeAlias, sourceConfig);
		});
		...
		it("should format complete page with all sections", async () => {
			vi.mocked(Tsdoc.summary).mockReturnValue("A type alias");
